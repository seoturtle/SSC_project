...
function Third() {
    ...
    useEffect(() => {
        if(value=="" || talkDelete == "") {
            setTimeout(() => {
                fetch("http://15.164.94.207:3002/stock_back/talk_content", {
                    method: "POST",
                    body: JSON.stringify({
                        user_idx: decode.idx,
                        code: storeCode
    ...
    useEffect(() => {
        if(talkDelete !=""){
            fetch("http://15.164.94.207:3002/stock_back/talk_delete", {
                method: "POST",
                body: JSON.stringify({
                    id: talkDelete
                }),
    ...
    const clickHandler = () => {
        fetch("http://15.164.94.207:3002/stock_back/talk_submit", {
            method: "POST",
            body: JSON.stringify({
                midx: decode.idx,
                name: decode.name,
    ...
    // useEffect(() => {
    //     if(likeTalkIdx != "") {
    //         fetch("http://15.164.94.207:3002/stock_back/like", {
    //         method: "POST",
    //         body: JSON.stringify({
    //             user_idx: decode.idx,
