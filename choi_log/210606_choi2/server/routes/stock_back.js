...
router.post('/high_low', (req, res) => {
    const code = req.body.code;
    var url = 'https://fchart.stock.naver.com/sise.nhn?symbol='+code+'&timeframe=week&count=52&requestType=0';
    var request = require('request');
    const convert = require('xml-js');
    request.get(url, function (error, response, body) {
        if(error) {
...
router.post('/stock_summary', (req, res) => {
    const code = req.body.code;
    const sql = 'SELECT stock_group, stock_total from stock2 WHERE stock_code = ?'
...
router.post('/recent_year', (req, res) => {
    const code = req.body.code;
    const sql = 'SELECT sales, bus_pro, net_pro from year20 where stock_code = ?'
...
router.post('/recent_quarter', (req, res) => {
    const code = req.body.code;
    const sql = 'SELECT sales, bus_pro, net_pro from quarter20_4 where stock_code = ?'
