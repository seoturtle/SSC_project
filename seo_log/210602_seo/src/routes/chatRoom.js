...
const socket = socketIoClient("http://15.164.94.207:5555", { autoConnect: false });
...
const MessageBox = (props) => {
    ...
    const postMessage = e => {
		...
		fetch("http://15.164.94.207:3002/search/add2", {
            method: "POST",
            body: JSON.stringify({
                oidx : props.oid,
                midx : props.midx
...
function ChatRoom({location}) {
	...
	const clickHandle = (e) => {
		fetch("http://15.164.94.207:3002/search/delete", {
            method: "POST",
            body: JSON.stringify({
                oidx : otherIdx,
                midx : myIdx
	...
	useEffect(() => {
        ...
        socket.on("message", (msg) => {
			if(midx == msg.midx && oid == msg.oid){
				addMessage(msg);
			}
            if(midx == msg.oid && oid == msg.midx){
				addMessage(msg);
			}
        });
